...
### B4 — `ProxyUserSheet` [SHIPPED — see rev 14]

**Deps**: B1 (parallel-safe with B2/B3) — shipped after B8.

**Scope (as shipped):**
- New `QlariusWeb.Components.ProxyUserSheet` LiveComponent with a three-step
  alias → data → confirm flow (no phone/OTP, no finalize) plus a `:creating`
  spinner and a `:complete` success step offering an "Add another" affordance
- Wired into `QlariusWeb.ProxyUsersLive` (the proxy-user list LV). `add_proxy`
  now opens the modal via `assign(:show_add_modal, true)` instead of
  `push_navigate(/register?mode=proxy&ref=...)`. New `close_add_modal` event
  + `handle_info({:proxy_user_created, user}, socket)` to refresh the list
  and flash on success.
- Referral inheritance via `Qlarius.Referrals.Context.from_admin/1` (generates
  and persists a referral code on the admin's me_file if missing — same
  ensure-code logic as the legacy flow, now reused).
- Uses the shared `QlariusWeb.Components.AuthSteps` sub-components
  (`alias_picker`, `data_step`, `confirm_step`) — the admin sees the exact
  same UI a real new user will see, which was the explicit reason for
  picking B4 up (UX fidelity for admin testing).
- Audit logging: emits `register_new_user.{allowed,denied}` via
  `Qlarius.Auth.AuditLog` with `surface: :admin_proxy_sheet` and
  `admin_user_id:` for traceability.
- Legacy `/register?mode=proxy` route intentionally left alive as a fallback
  (no admin UI links to it anymore from ProxyUsersLive).

**Intentional UX divergence from the legacy `/register?mode=proxy` flow:**
the new sheet does **NOT** auto-activate the proxy on creation. Legacy flow
calls `Accounts.activate_proxy_user/2` which deactivates any currently-active
proxy and switches the admin's scope into the new user — a confusing jump
when the admin's mental model is "I'm managing my list". New flow creates
the proxy `active: false` (via `maybe_insert_proxy_user/2`'s default) and
leaves the admin unchanged; they toggle explicitly from the list if they
want to switch in.

**Duplication note (deferred DRY):** the alias/data/confirm event handlers
and the `build_user_attrs / advance / retreat / validate_birthdate /
alias_ready? / data_step_ready? / can_complete? / init_signup_assigns /
regenerate_key / load_sex_options` helpers are parallel to the sign-up half
of `AuthSheet`. With only two consumers we're keeping them forked rather
than extracting a shared `SignupStateMachine` module; if a third consumer
appears, extract. Module-level moduledoc on `ProxyUserSheet` flags this.

**Files:**
- new: `lib/qlarius_web/components/proxy_user_sheet.ex` (~500 loc, single
  file — no separate heex template; inline `render_state/1` per state)
- edit: `lib/qlarius_web/live/proxy_users_live.ex` — `add_proxy` handler
  swapped, `close_add_modal` + `handle_info({:proxy_user_created, _}, _)`
  added, sheet mounted in render. `JS` alias added.

**Ship criterion (met):** admin creates proxy user entirely in-modal from
`/proxy_users`; proxy list updates without page reload; admin stays signed
in throughout; referral code inherited from admin (generated if missing).
...
- ~~Audit logging~~ — shipped rev 13
- ~~Cloudflare Turnstile or similar on `send_code` after N failures in a session~~ — **deferred indefinitely** rev 13 (see rationale); concrete design on shelf if aggregate audit logs indicate distributed abuse
...
- new: `lib/qlarius/auth/audit_log.ex` (structured Logger wrapper — `log/2`, `mask_phone/1`; 8 event types covering `send_code` / `verify_code` / `register_new_user` / `finalize_session` allowed+denied)
- new: `test/qlarius/auth/rate_limit_test.exs`
- new: `test/qlarius/auth/audit_log_test.exs`
- edit: `lib/qlarius_web/components/auth_sheet.ex` — `client_ip` attr, gates in `send_code`, `rate_limited` reason branch in `auth:finalize_failed`, all decision branches emit `AuditLog.log/2`
- edit: `lib/qlarius_web/controllers/auth/finalize_session_controller.ex` — per-IP gate before token verify; returns `429 {"error": "rate_limited"}`; all branches emit `AuditLog.log/2`
