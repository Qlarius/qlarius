...
**Scope:**
- Flip `:qlink_anon` live_session → `:mount_current_scope`
- Swap `:browser_anon` pipeline → full `:browser` for qlinkin.bio host scope
- Mount auth-related controller routes under qlinkin.bio host scope (specifically `/auth/finalize_session`, `/auto_login/:token` for `/login` fallback, `/users/log_out`, rate-limit endpoints)
- Cloudflare: add cache rule "bypass cache when `_qlarius_key` cookie is present on qlinkin.bio"
- Enable `:auth_sheet[:on_qlinkin_bio]` flag
- Remove `is_anon_surface` assign and all its dependent branches from `QlinkPage.Show` and templates

**Files:**
- edit: `lib/qlarius_web/router.ex` (qlinkin.bio host scope)
- edit: `lib/qlarius_web/live/qlink_page/show.ex` + `.html.heex`
- edit: `lib/qlarius_web/user_auth.ex` (possibly retire `:mount_anonymous_scope`)
- external: Cloudflare cache rule

**Risk:** cache miss storm at cutover for any auth'd qlinkin.bio visitors. Mitigate with gradual rollout during low-traffic window, monitor origin load.

**Ship criterion:** signing in on qlinkin.bio works with in-place completion; sessions persist across page loads; anon viewers still hit edge cache.
...
**Scope:**
- Hammer rate limits on `send_code`: per-phone (3/10min), per-IP (10/hour)
- Hammer rate limits on `POST /auth/finalize_session`: per-IP (20/hour) to prevent token brute-forcing
...
- edit: `lib/qlarius_web/components/auth_sheet.ex` (hammer gates on `send_code`)
- edit: `lib/qlarius_web/controllers/auth/finalize_session_controller.ex` (rate limit)
