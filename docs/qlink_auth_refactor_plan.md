...
### B6 — `qlinkin.bio` fully authed surface [STAGE 1 SHIPPED — see rev 9]
...
**Stage 2 (prod cutover, NOT yet done):**
- Add Cloudflare cache rule: "bypass cache when the `_qlarius_key` cookie is present on qlinkin.bio". Required so authed visitors don't see cached anonymous HTML after `AuthSheet` sign-in. Without this rule, production responses may be served stale and the authed UI won't appear.
- Flip `config :qlarius, :auth_sheet, on_qlinkin_bio: true` in production env config (or set `QLARIUS_AUTHSHEET_ON_QLINKIN_BIO=true` at runtime, depending on final runtime-config wiring).
- Optionally revisit `:browser_anon` pipeline retirement — still used by the `qadabra.app` apex-redirect scope in the router, so leaving it in place is harmless.
