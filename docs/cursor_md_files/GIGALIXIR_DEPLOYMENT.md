...
# Deployment hostname (Gigalixir app URL). User-facing links (referrals,
# gifts, Endpoint.url/0) use the canonical :public_app_host (qadabra.app).
gigalixir config:set PHX_HOST="qlarius.gigalixirapp.com"
...
| `PHX_HOST` | Deployment hostname (Gigalixir) | `qlarius.gigalixirapp.com` |

Public referral, gift, and share links use `qadabra.app` via `:public_app_host` in `config/config.exs` (not `PHX_HOST`).
