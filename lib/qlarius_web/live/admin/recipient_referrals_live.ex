defmodule QlariusWeb.Admin.RecipientReferralsLive do
  use QlariusWeb, :live_view

  alias Qlarius.Referrals
  alias Qlarius.Sponster.Recipients

  def mount(%{"id" => recipient_id}, _session, socket) do
    recipient = Recipients.get_recipient!(recipient_id)
    referred_users = Referrals.list_referrals_for_referrer("recipient", recipient.id)

    my_referral_code =
      recipient.referral_code || Referrals.generate_referral_code("recipient")
    ...
    socket =
      socket
      |> assign(:page_title, "Referrals - #{recipient.name}")
    ...
    <div class="container mx-auto px-4 py-6">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold dark:text-white">
          Referrals - {@recipient.name}
        </h1>
        <.link navigate={~p"/admin/recipients/#{@recipient.id}"} class="btn btn-ghost">
          <.icon name="hero-arrow-left" class="h-5 w-5" /> Back to Recipient
        </.link>
      </div>
