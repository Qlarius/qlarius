defmodule QlariusWeb.MeCPConnectorsPanel do
  @moduledoc """
  Connector onboarding: create a MeCP grant (scope, tier, budget), get the
  grant-bound token to paste into an MCP client, rotate or revoke it later.
  Tokens are shown exactly once.
  """
  ...
  def render(assigns) do
    ~H"""
    <div class="pb-8 w-full max-w-2xl mx-auto flex flex-col gap-4" id={@id}>
      ...
      <div :if={@new_token} class="card bg-base-200 border border-success">
        <div class="card-body gap-3">
          ...
          <div class="card-actions justify-end">
            <button class="btn btn-sm btn-ghost rounded-full" phx-click="dismiss_token" phx-target={@myself}>
              Done
            </button>
          </div>
        </div>
      ...
      <div :for={grant <- @grants} class="card bg-base-100 border border-base-300">
        <div class="card-body py-4 gap-2">
          <div class="flex items-center justify-between">
            ...
            <span class={"badge rounded #{badge}"}>{label}</span>
          </div>
      ...
      >
        <div class="card-body gap-4">
          ...
          <div class="card-actions justify-end">
            ...
            <button type="submit" class="btn btn-primary rounded-full">Create and get token</button>
          </div>
        </div>
      </form>
    </div>
