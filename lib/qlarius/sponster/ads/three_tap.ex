defmodule Qlarius.Sponster.Ads.ThreeTap do
  ...
  def create_banner_ad_event(
    ...
    ad_event_attrs = %{
      ...
      event_me_file_collect_amount: phase.pay_to_me_file_fixed,
    ...
    ad_event_attrs =
      if recipient && split_amount > 0 do
        ...
        split_amount_to_recipient =
          ad_event_attrs.event_me_file_collect_amount
          |> Decimal.mult(split_percentage)
          |> Decimal.round(2, :down)

        adjusted_me_file_collect_amt =
          Decimal.sub(ad_event_attrs.event_me_file_collect_amount, split_amount_to_recipient)

        Map.merge(ad_event_attrs, %{
          recipient_id: recipient.id,
          event_split_code: recipient.split_code,
          event_recipient_split_pct: split_amount,
          # update the recipient collect amt to be the event_me_file_collect_amount - minus the split_pct amount
          event_recipient_collect_amt: split_amount_to_recipient,
          event_me_file_collect_amount: adjusted_me_file_collect_amt,
  ...
  def create_jump_ad_event(
    ...
    event_me_file_collect_amount =
      Decimal.sub(offer.amount, previous_phase.pay_to_me_file_fixed)

    event_sponster_collect_amt =
      Decimal.sub(event_marketer_cost_amt, event_me_file_collect_amount)
    ...
    ad_event_attrs = %{
      ...
      event_me_file_collect_amount: event_me_file_collect_amount,
  ...
  def calculate_offer_totals(offer_id, recipient \\ nil) do
    ...
    me_file_collect_total =
      Enum.reduce(ad_events, Decimal.new("0.00"), fn event, acc ->
        Decimal.add(acc, event.event_me_file_collect_amount || Decimal.new("0.00"))
      end)
