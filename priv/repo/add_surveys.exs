defmodule Seeds do
  ...
  def run do
    ...
    insert_surveys()
    update_trait_questions()
    update_trait_value_answers()
    insert_traits_surveys()

    for table <- ~w[survey_categories surveys] do
  ...
  defp trait_question_data do
    # This is the original SurveyQuestion data from the Rails app but the only
    # thing we care about in our simplified data model is the text.
    # This is in its own function because we need it when importing
    # both 'questions' and 'trait questions'
    [
      %{
        id: 1,
        trait_id: 94,
  ...
  defp update_trait_questions do
    trait_question_data()
    |> Enum.each(fn attrs ->
      trait = Repo.get!(Trait, attrs.trait_id)

      question =
        if trait.name == attrs.text do
          nil
        else
          attrs.text
        end

      trait |> change(%{question: question}) |> Repo.update!()
    end)
  end

  defp insert_traits_surveys do
    now = DateTime.utc_now() |> DateTime.truncate(:second)

    question_id_to_trait_id =
      trait_question_data()
      |> Enum.map(& {&1.id, &1.trait_id})
      |> Enum.into(%{})
    ...
    |> Enum.map(fn data ->
      case Map.fetch(question_id_to_trait_id, data.question_id) do
        {:ok, trait_id} ->
          %{
        ...
        :error ->
          # I'll worry later about getting all the data perfect
          nil
      end
    end)
    |> Enum.filter(& !is_nil(&1))
    |> then(&Repo.insert_all("traits_surveys", &1))
  end

  defp update_trait_value_answers do
    # Original SurveyAnswer data; see comment above about SurveyQuestions
    [
      %{
        id: 1,
        question_id: 1,
    ...
    |> Enum.each(fn attrs ->
      value = Repo.get!(TraitValue, attrs.trait_value_id)

      answer =
        if value.name == attrs.text do
          nil
        else
          attrs.text
        end

      value |> change(%{answer: answer}) |> Repo.update!()
    end)
  end
end
